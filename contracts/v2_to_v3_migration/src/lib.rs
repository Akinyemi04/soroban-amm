...
//!   6. Optionally apply a fee discount (set at construction by the admin).
...
use concentrated_liquidity::{ClPoolClient, tick_to_sqrt_price_x96};
...
pub enum DataKey {
    ...
    TokenA,
    TokenB,
    FeeDiscountBps, // basis-point discount on V3 entry fee during migration
}
...
impl V2ToV3Migration {
    ...
    ///
    /// `min_amount_a` / `min_amount_b` provide slippage protection: the
    /// transaction reverts if the V2 burn yields less than these amounts.
    pub fn migrate(
        env: Env,
        lp: Address,
        lp_shares: i128,
        ...
        // 1. Pull V2 LP shares from caller into this contract
        TokenClient::new(&env, &v2_lp_addr).transfer(&lp, &contract_addr, &lp_shares);

        // 2. Burn V2 LP shares → receive token_a and token_b
        let v2 = AmmPoolClient::new(&env, &v2_pool_addr);
        let (received_a, received_b) = v2.remove_liquidity(&contract_addr, &lp_shares, &min_amount_a, &min_amount_b);
        ...
        // 4. Estimate liquidity from amounts
        let liquidity = Self::amounts_to_liquidity(received_a, received_b, &range);
        assert!(liquidity > 0, "computed liquidity is zero");

        // 5. Approve V3 pool to pull tokens from this contract
        let v3 = ClPoolClient::new(&env, &v3_pool_addr);
        let mint_result = v3.mint_position(
            &contract_addr,
            &range.lower_tick,
            &range.upper_tick,
            &liquidity,
        ...
        // 6. Compute dust (tokens not consumed by CL deposit)
        ...
        // 8. Transfer the V3 position NFT (LP token) to the caller
        // The CL pool minted LP tokens to this contract; forward them.
        let v3_lp_bal = TokenClient::new(&env, &v3_pool_addr).balance(&contract_addr);
        if v3_lp_bal > 0 {
            TokenClient::new(&env, &v3_pool_addr).transfer(&contract_addr, &lp, &v3_lp_bal);
        }
        ...
        env.events().publish(
            ...
            (
                lp_shares,
                mint_result.position_id,
                mint_result.amount_a,
                mint_result.amount_b,
                dust_a,
                dust_b,
            ),
        );
    ...
    /// The range centres on the current pool price with width proportional to
    /// the token ratio, giving ±10 % price coverage by default.
    pub fn preview_range(env: Env, amount_a: i128, amount_b: i128) -> RangeResult {
        ...
        // Width: 10 % of the price range around the current tick (≈ 1000 ticks).
        // Wider if the ratio suggests skewed price (one side has much more value).
        let ratio_skew = if amount_a > 0 && amount_b > 0 {
            // skew in the range 0..10; higher = wider spread for the richer side
        ...
        let estimated_liquidity =
            Self::amounts_to_liquidity_static(amount_a, amount_b, lower_tick, upper_tick, state.sqrt_price_x96);
    ...
    fn amounts_to_liquidity(amount_a: i128, amount_b: i128, range: &RangeResult) -> i128 {
        // Simplified: return pre-computed estimate from preview_range
        range.estimated_liquidity.max(1)
    }

    fn amounts_to_liquidity_static(
        amount_a: i128,
        amount_b: i128,
        lower_tick: i32,
        upper_tick: i32,
        ...
        if sqrt_price <= sqrt_lower {
            // All token_a
            if sqrt_lower == sqrt_upper || sqrt_lower == 0 {
                return 0;
            }
            amount_a
                .checked_mul(sqrt_lower)
                .unwrap_or(i128::MAX)
                .checked_div(sqrt_upper - sqrt_lower)
                .unwrap_or(1)
                .max(1)
        } else if sqrt_price >= sqrt_upper {
            // All token_b
            use concentrated_liquidity::Q96;
            ...
            // In range: take minimum of two estimates
            use concentrated_liquidity::Q96;
            let liq_a = if sqrt_price > 0 && sqrt_upper > sqrt_price {
                amount_a
                    .checked_mul(sqrt_price)
                    .unwrap_or(i128::MAX)
                    .checked_div(sqrt_upper - sqrt_price)
                    .unwrap_or(1)
            } else {
                1
            };
