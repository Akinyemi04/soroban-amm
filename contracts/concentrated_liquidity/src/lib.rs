...
/// Full Uniswap MAX would exceed i128; we cap at a safe value for Soroban.
...
pub enum DataKey {
    ...
    CurrentTick,
    ActiveLiquidity,
    FeeGrowthGlobalA, // accumulated fee per unit liquidity for token_a
    FeeGrowthGlobalB,
    FeeBps,
    Positions,  // Map<(Address, i32, i32), Position>
    TicksMap,   // Map<i32, TickInfo>
    NextPosId,
}
...
pub struct Position {
    ...
    pub upper_tick: i32,
    pub liquidity: i128,
    /// fee_growth_inside snapshot at last update (scaled by Q96)
...
/// Uses the identity: sqrt_price_x96 ≈ Q96 * 1.00005^tick (first-order approx).
/// For correctness in production, a proper tick-sqrt price table is needed;
/// this approximation suffices for fuzz property checks.
pub fn tick_to_sqrt_price_x96(tick: i32) -> i128 {
    // Each tick step is ×sqrt(1.0001) ≈ ×1.00005.
    // We use: sqrt_price = Q96 * (1.00005)^tick
    // Implemented via repeated squaring is too expensive; use linear approx:
    //   ln(sqrt_price/Q96) = tick * ln(sqrt(1.0001)) ≈ tick * 0.000049995
    // We stay in integer math by computing the ratio incrementally.
    // For the fuzz harness the exact value is less important than monotonicity.
    let base: i128 = Q96;
    if tick == 0 {
        return base;
    }
    // Scale factor per tick: 100001/100000 for positive direction.
    // We use a 64-bit fixed-point multiply to avoid overflow.
    let abs_tick = tick.unsigned_abs() as i128;
    // Each tick multiplies by (1 + 1/20000) or divides; cap at 1000 steps to
    // avoid accumulated overflow — callers validate tick range.
    let steps = abs_tick.min(1000);
    let mut result = base;
    for _ in 0..steps {
        if tick > 0 {
            // ×(1 + 1/20000) approximation
...
impl ClPool {
    ...
    /// Mint a new position in [lower_tick, upper_tick) with the given liquidity.
    /// Returns actual token amounts deposited and the new position ID.
    ...
    /// Burn a position entirely, returning tokens proportional to liquidity.
    pub fn burn_position(env: Env, owner: Address, position_id: i128) -> BurnResult {
        ...
        // Accrue uncollected fees (rounded down — never owe more than earned)
    ...
    /// `zero_for_one`: true = swap token_a for token_b (price goes down).
    pub fn swap(
        env: Env,
        trader: Address,
        token_in: Address,
        ...
        // Simple single-step swap (no partial tick crossing for brevity;
        // tick-crossing loop handled below for up to 10 crossings).
        ...
        while remaining > 0 && crossings < 10 {
            ...
            let (step_out, new_sqrt_price, consumed) = if active_liquidity > 0 {
                ...
                // No liquidity in range — jump to next tick
            ...
            if active_liquidity > 0 && consumed > 0 {
                let fee_per_liquidity = fee_amount.min(consumed) * Q96 / active_liquidity.max(1);
                if zero_for_one {
                    fg_a += fee_per_liquidity;
                } else {
                    fg_b += fee_per_liquidity;
                }
            }

            // Check if we crossed the tick
            let crossed = if zero_for_one {
                sqrt_price <= sqrt_next || sqrt_price <= price_limit
            } else {
                sqrt_price >= sqrt_next || sqrt_price >= price_limit
            };

            if crossed && next_tick != current_tick {
                // Cross the tick: flip liquidity_net
                ...
                if let Some(mut tick_info) = ticks.get(next_tick) {
                    // Flip fee growth outside on crossing
    ...
    /// Collect accrued fees for a position. Returns (fee_a, fee_b) transferred.
    pub fn collect_fees(env: Env, owner: Address, position_id: i128) -> (i128, i128) {
        ...
        // Tokens owed rounded down (never pay out more than accrued)
        ...
        pos.tokens_owed_a = 0;
        pos.tokens_owed_b = 0;
        positions.set(position_id, pos.clone());
        env.storage().instance().set(&DataKey::Positions, &positions);
    ...
    /// Approximate tick from sqrt_price_x96 (inverse of tick_to_sqrt_price_x96).
    fn sqrt_price_to_tick(sqrt_price_x96: i128) -> i32 {
        ...
        // tick ≈ log(sqrt_price_x96 / Q96) / log(sqrt(1.0001))
        // ≈ (sqrt_price_x96 - Q96) / Q96 * 20000 (linear approx near Q96)
        let ratio = (sqrt_price_x96 - Q96) * 20_000 / Q96;
        (ratio as i32).clamp(MIN_TICK, MAX_TICK)
    }

    /// Compute token amounts for a given liquidity amount and price range.
    fn liquidity_to_amounts(
        ...
        if sqrt_price <= sqrt_lower {
            // Current price below range — all token_a
            amount_a = liquidity
            ...
            // Current price above range — all token_b
            amount_a = 0;
            amount_b = liquidity
                .checked_mul(sqrt_upper - sqrt_lower)
                .unwrap_or(i128::MAX)
    ...
    /// Single swap step: compute output tokens and new sqrt_price.
    /// Returns (amount_out, new_sqrt_price, amount_in_consumed).
    fn compute_swap_step(
        ...
        let (amount_out, new_price) = if zero_for_one {
            // Selling token_a: price goes down toward sqrt_price_target
            // Δtoken_b = L * (sqrt_current - sqrt_next) / Q96
            // Δtoken_a (in) = L * Q96 * (1/sqrt_next - 1/sqrt_current)
            // Compute how far we can go with amount_remaining
            ...
            let out = liquidity
                .checked_mul(sqrt_price_current - new_p)
                .unwrap_or(0)
                / Q96;
            (out.max(0), new_p)
        } else {
            // Selling token_b: price goes up toward sqrt_price_target
        ...
        let consumed = amount_remaining; // simplified: consume all input in one step
        (amount_out, new_price, consumed)
    }

    /// Find next initialized tick in the given direction (linear scan, max 200 ticks).
    ...
    fn update_tick(env: &Env, tick: i32, liquidity_delta: i128, is_lower: bool) {
        ...
        // Lower tick: liquidity_net += delta; upper tick: liquidity_net -= delta
        if is_lower {
    ...
    /// Compute fee growth inside a tick range at the current tick.
